...
import {TieSecondaryMotion} from './tie-secondary-motion.mjs';
...
export function mountReceptionist(stage,{player,posterURL=asset('studio-poster.png'),parentOrigin=null,onReport=()=>{},beforeAnimation=null}={}){
 ...
 let staticPose=null,staticBlink=null,loadStarted=false,tieMotion=null;
 try{
 ...
 document.addEventListener('visibilitychange',()=>{if(document.hidden)player?.stop();else tieMotion?.reset();},{signal});
 ...
 async function load({avatar=asset('Prime_Reception_Native_Web.glb'),office=asset('Reception_Office_Environment.glb')}={}){
  ...
  model.traverse(o=>{
   ...
   // GLTFLoader sanitizes periods from bone names (eyeball.L -> eyeballL).
   const boneName=({'eyeballL':'eyeball.L','eyeballR':'eyeball.R'})[o.name]||o.name;
   if(o.isBone&&['head','neck','eyeball.L','eyeball.R'].includes(boneName))bones.set(boneName,{bone:o,rest:o.quaternion.clone()});
  });
  const face=meshes.find(o=>o.name==='PrimeReception_Face');
  if(!face||!bones.has('head')||!bones.has('eyeball.L')||!bones.has('eyeball.R'))throw Error('Receptionist rig is incomplete');
  ...
  const nativeBob=Boolean(model.getObjectByName('Prime_Bob_Hair'));
  adapter.profile=nativeBob?'native':'legacy';
  tieMotion=new TieSecondaryMotion(model);report.tie=tieMotion.report;
  report.character=nativeBob?'Native Human Generator A / side-part bob':'Earlier receptionist fallback';
  // Preserve the selected native expression; the old derivative used a smaller baseline.
  base('mouthSmileLeft',nativeBob ? .28 : .055);base('mouthSmileRight',nativeBob ? .28 : .055);
 ...
 function frame(now){
  ...
  if(!document.hidden){
   ...
   if(model){
    beforeAnimation?.({model,dt,elapsed});
    ...
    // Accessory pass runs after all character motion and never writes host bones.
    tieMotion?.update(dt,{reducedMotion:reduced.matches});
 ...
 raf=requestAnimationFrame(frame);
 function dispose(){if(disposed)return;disposed=true;player?.stop();tieMotion?.dispose();cancelAnimationFrame(raf);observer.disconnect();events.abort();
 ...
 return {load,dispose,report,scene,camera,mixer,adapter,player,get tieMotion(){return tieMotion;},setPose(v){staticPose=v;},setBlink(v){staticBlink=v;},setPointer};
}
