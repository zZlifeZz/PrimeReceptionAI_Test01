...
async function raw(action,body={}){let httpStatus=0;try{const r=await fetch(BASE+action,{method:'POST',headers:{'content-type':'application/json',...(token?{authorization:'Bearer '+token}:{})},body:JSON.stringify(body),signal:AbortSignal.timeout(65000),credentials:'omit',redirect:'error'});httpStatus=r.status;const data=await r.json();if(!r.ok){console.error('Prime connection HTTP',{action,httpStatus,code:typeof data.error==='string'?data.error.slice(0,80):'request_failed'});throw Error(unavailable);}return data;}catch(e){console.error('Prime connection failed',{action,httpStatus,errorType:e.name});throw Error(unavailable);}}
