...
function fail(){idle();status.textContent=unavailable;clearTimeout(errorTimer);errorTimer=setTimeout(()=>status.textContent='',6500);}
...
stopButton.onclick=()=>cancel().catch(fail);
...
window.addEventListener('message',async e=>{
 ...
 if(d.type==='prime-ready'){ready=true;post('prime-sound',{enabled:window.PrimeSite?.state.soundOn!==false});post('prime-attention',{state,...focus});return;}
 if(d.type==='prime-audio-ready'){audioReady=true;root.dataset.voice='enabled';return;}
 ...
 if(ack==='speaking'){face('speaking');status.textContent='';}else idle();
 try{await raw('ack',{...ref,state:ack});if(ref!==active)return;if(ack!=='speaking'){clear();if(ack==='error')fail();}}catch{if(ref===active){clear();fail();}}
});
...
function sectionChanged(section){
 ...
 if(!labels[section]){cancel().catch(fail);return;}
...
window.addEventListener('prime:sound',e=>{if(typeof e.detail?.enabled==='boolean'){post('prime-sound',{enabled:e.detail.enabled});if(!e.detail.enabled)cancel().catch(fail);}});
