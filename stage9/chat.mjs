...
async function raw(action,body={}){const r=await fetch(BASE+action,{method:'POST',headers:{'content-type':'application/json',...(token?{authorization:'Bearer '+token}:{})},body:JSON.stringify(body),signal:AbortSignal.timeout(65000),credentials:'omit',redirect:'error'});if(!r.ok)throw Error(unavailable);return r.json();}
async function initialize(){if(sessionId)return;if(initializing)return initializing;initializing=(async()=>{const s=await raw('session');if(s.scope!=='test-ui-only'||!s.token||!s.sessionId)throw Error(unavailable);token=s.token;sessionId=s.sessionId;post('prime-bind',{sessionId});})();try{await initializing;}finally{initializing=null;}}
